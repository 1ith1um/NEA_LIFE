...
// including functions from the other files
mod database;
mod graph;
mod preset;
...
// overall object which houses the entire simulation
pub struct LifeSim {
    grid: Grid,
    is_playing: bool,
    queued_ticks: usize,
...
impl LifeSim {
    // implementing general functions for the LifeSim object, managing the whole simulation

    fn new() -> Self {
        let db = database::WorldDatabase::new("life_simulation.db").unwrap();
        let saves = db.list_saves().unwrap_or_default();
        let selected_save = saves.first().cloned();
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::Tick | Message::Next => {
                ...
                let organism_count = self.grid.state.life.organisms.len();
            ...
            Message::SaveState => {
                ...
                if let Err(e) = state.validate() {
                    println!("Invalid state: {}", e);
                    return Task::none();
                }

                // FIX: proper error handling
                match self.db.save_state(&state, None) {
                    Ok(id) => {
                        println!("Saved successfully! ID: {}", id);
                    }
                    Err(e) => {
                ...
                // Refresh saves
                self.saves = self.db.list_saves().unwrap_or_default();
                self.selected_save = self.saves.first().cloned();
            }

            Message::LoadState => {
                if let Some(ref info) = self.selected_save.clone() {
                    if let Ok(state) = self.db.load_state_by_id(info.id) {
            ...
            Message::SelectSave(info) => {
                self.selected_save = Some(info);
            }
            Message::Ignore => {}
        }
        Task::none()
...
fn view_controls<'a>(
    ...
    // Only enable Load when a save slot is actually selected.
    let load_button = if selected_save.is_some() {
    ...
    let save_picker = {
        row![
            pick_list(saves, selected_save, Message::SelectSave),
            load_button,
        ]
        .spacing(10)
    };
...
pub mod grid {
    ...
    impl Grid {
        ...
        pub fn from_preset(preset: Preset) -> Self {
            ...
            let organisms = Life::find_all_organisms(cells.clone());
            // gathers all the organisms from the preset using a depth first search

            // instantiates a grid using these organisms and cells, which are instantiated into a life object

            Self {
                state: State::with_life(Life { cells, organisms }), // instantiate a life object from cells and gathered organisms
                preset,
        ...
        pub fn tick(&mut self, amount: usize) -> Option<impl Future<Output = Message>> {
            // tick function allows a single tick to occur
    ...
    impl canvas::Program<Message> for Grid {
        ...
        fn draw(
            ...
            let life = self.life_cache.draw(renderer, bounds.size(), |frame| {
                ...
                frame.fill(&background, Color::from_rgb8(0x38, 0x39, 0x3B));
                ...
                frame.with_save(|frame| {
                    ...
                    for cell in region.cull(self.state.cells()) {
                        // give different colours for different cell types
    ...
    // Object which encapsulates the current state of the simulation
    ...
    impl State {
        pub fn with_life(life: Life) -> Self {
            // instantiates a new State with a given Life
            Self {
                life,
                ..Self::default()
            }
    ...
    impl Organism {
        ...
        fn new(cells: Vec<Cell>) -> Self {
            // instantiates a default organism from a set of cells, using constant NEW_ORGANISM_ENERGY
            Organism {
                id: None,
                cells: cells.clone(),
        ...
        pub fn adjacent_food(&self, life: &Life) -> Vec<Cell> {
            ...
            for cell in &self.cells {
                // iterate over each cell in the organism
                for neighbor in Cell::neighbors(*cell) {
                    // iterate over each neighbour of each cell
                    if let Some(food_cell) = life.cells.iter().find(|c| **c == neighbor) {
                        if food_cell.cell_type == CellType::Food && !self.contains(food_cell) {
                            if !food.contains(food_cell) {
                                // if there is a cell of food type which is not contained in the organism,
                                // and also not already visited, add it to the list of food surrounding
                                food.push(*food_cell);
                            }
                        }
                    }
        ...
        // Calculates the limits of an organism, useful for calculations
        pub fn bounding_box(&self) -> (isize, isize, isize, isize) {
            //iterates over each cell in the organism, finding the minimum and maximum i and j values for each
    ...
    // Life object stores a Vector (list)
    #[derive(Clone, Default)]
    pub struct Life {
        pub cells: Vec<Cell>, // a list of cells, effectively all the cells on the grid
        pub organisms: Vec<Organism>, // A list of (lists of cells) - organisms
    }

    // implementing functions for the life struct (object)

    use std::collections::HashSet;
    impl Life {
        const CHANCE_TO_EAT: f64 = 80.0; // chance for an organism to eat surrounding food
        const MUTATION_RATE: f64 = 50.0; // Chance for an organism to mutate an extra cell
        const CHANCE_TO_SKIP_REPRODUCTION: f64 = 60.0; // chance for an organism to skip the reproduction step on a certain tick
        const CHANCE_TO_GROW_FOOD: f64 = 1.0; // Chance for a grower to grow food on a given tick
        const ENERGY_FROM_FOOD: usize = 5; // energy given to an organism from a single cell of food
        const REPRODUCTION_COST: usize = 200; // how much energy it takes for an organism to reproduce
        const REPRODUCTION_GAP: isize = 1; // Gap between original organism and its offspring

        fn build_occupied(&self) -> HashSet<(isize, isize)> {
        ...
        // returns the amount of cells on the grid
        fn len(&self) -> usize {
            self.cells.len()
        }

        // returns true or false if the grid contains that cell
        fn contains(&self, cell: &Cell) -> bool {
            self.cells.contains(cell)
        }

        // pushes that cell to the grid if it is not already on the grid
        fn populate(&mut self, cell: Cell) {
        ...
        // takes away that cell from the grid
        fn unpopulate(&mut self, cell: &Cell) {
            self.cells.retain(|c| c != cell);
        }

        fn create_organism_from_cell_clump(cells: Vec<Cell>, start_cell: Cell) -> Organism {
            // Uses a queue and a Depth First search to generate an organism from a different start cell
            // Searches in all possible directions around the start cell, utilising a circular queue and a visited list
            // similar to find_all_organisms
            // not currently utilised but left in for future possible uses
            ...
            while let Some(cell) = queue.pop_front() {
                ...
                for &(dx, dy) in &directions {
                    ...
                    // Check if neighbor is a valid cell and not visited
        ...
        pub fn grow_food(&mut self) {
            // grow_food function searches the grid for grower cells
            // for each grower cell it finds it iterates over the four cardinal directions around the cell
            // if the neighbours of the grower cell are empty the chance function will run
            // according to the constant CHANCE_TO_GROW_FOOD
            // if it returns true, food growth will be skipped.
            // if not, food will be placed around the cell
            ...
            for cell in &self.cells {
                ...
                for cell in &self.cells {
                    if cell.cell_type == CellType::Grower {
                        ...
                        for &(dx, dy) in &directions {
                            let new_pos = (cell.i + dy, cell.j + dx);
            ...
            // extend the grid with new food cells
            self.cells.extend(new_food_cells);
        }

        fn chance(percentage: f64) -> bool {
            // function that given a percentage returns true or false randomly according to that percentage
            // placed in a separate function as to encapsulate and save redundant code reuse.
            use rand::Rng;
        ...
        fn consume_food(&mut self) {
            // consume_food function
            // for each organism on the grid
            // iterates through each of it's cells
            // if a food cell is detected around any of the organism's cells
            // the chance function is run with constant CHANCE_TO_EAT
            // if the chance function returns true,
            // the organism's energy is incremented by the constant ENERGY_FROM_FOOD
            // the position of the food cell to be removed is passed onto the vector of cells to be removed
            // after all food cells to be removed have been found
            // iterates through each cell to be removed and removes them from the grid
            ...
            for organism in &mut self.organisms {
                ...
                for cell in &organism.cells {
                    let directions = [(0, 1), (0, -1), (1, 0), (-1, 0)]; // directions
        ...
        fn find_all_organisms(cells: Vec<Cell>) -> Vec<Organism> {
            // find_all_organisms function
            // at the moment used to define organisms for given presets
            // Uses a Breadth first Search across a list of Cell objects to find cells that are
            // clumped together contiguously, avoiding already visited cells.
            // keeps track of cells to avoid producing duplicate organisms
            // returns a list of organisms, gathered from the cells.
            ...
            for &cell in &cells {
                ...
                while let Some(current) = queue.pop_front() {
                    ...
                    for &(dx, dy) in &directions {
                        ...
                        // check if it is a valid cell and hasn't been visited
                        if cells.contains(&neighbor) && !visited.contains(&neighbor) {
                            queue.push_back(neighbor);
                            visited.push(neighbor);
                        }
        ...
        fn random_mutation() -> Option<CellType> {
            // returns a random CellType when run
        ...
        pub fn reproduce_organism(&mut self) {
            ...
            for organism in &mut self.organisms {
                ...
                // use bounding_box to calculate the dimensions
    ...
    // implementing trait fromIterator for Life, allowing us to create a Life object from an iteration over a list of Cells
    ...
    impl Cell {
        ...
        fn at(position: Point) -> Cell {
            // function used to find the cell in which the mouse is hovering over
